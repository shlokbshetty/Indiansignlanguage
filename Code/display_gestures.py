...
def create_montage():
    ...
    gestures = [os.path.basename(g)
                for g in gesture_folders if os.path.isdir(g)]
    ...
    for i in range(GRID_ROWS):
        ...
        for gesture_name in row_gestures:
            ...
            if not img_list:
                ...
                print(
                    f"Warning: No images found in '{gesture_name}'. Using black square.")
                ...
                if img is None:
                    print(
                        f"Warning: Could not read {img_path}. Using black square.")
                    img = np.zeros((image_y, image_x), dtype=np.uint8)
        ...
        if col_img is None:
            continue  # Skip if this row had no gestures
